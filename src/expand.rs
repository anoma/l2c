use crate::expr::{
    ContinuationExpr, Expr, InvokeExpr, JumpExpr, LiteralExpr, MetaExpr, StorageExpr, parse_expr,
};
...
pub fn init_sexprs(env: &mut HashMap<String, Expr>) {
    let nil = Expr::Meta(MetaExpr {
        reference: None,
        fragment: SExpr::List(vec![]),
    });
    env.insert("nil".to_string(), nil);
    for i in 33u8..127u8 {
        let reference = vec!['-', i as char, '-'].into_iter().collect();
        let sexpr = Expr::Meta(MetaExpr {
            reference: None,
            fragment: SExpr::Char(i as char),
        });
...
fn eval_invoke(
    ...
    global_env: &HashMap<String, Expr>,
    args: Vec<Expr>,
) -> Result<Expr, JumpBuffer> {
    let i = args.len();
    if i < invoke.arguments.len() {
        ...
        eval(
            ...
            env,
            global_env,
            Rc::new(|reference, counter| match (reference, &args[..]) {
                (Expr::Symbol(sym), [Expr::Literal(a), Expr::Literal(b)])
                    if sym.reference == "+" =>
                {
                ...
                {
                    Ok(Expr::Literal(LiteralExpr {
                        value: a.value.wrapping_div(b.value),
                    }))
                }
                (Expr::Symbol(sym), [Expr::Literal(a), Expr::Literal(b)])
                    if sym.reference == "%" =>
                {
                    Ok(Expr::Literal(LiteralExpr {
                        value: a.value.wrapping_rem(b.value),
                    }))
                }
                (Expr::Symbol(sym), [Expr::Literal(a), Expr::Literal(b)])
                    if sym.reference == "==" =>
                {
                    Ok(Expr::Literal(LiteralExpr {
                        value: u32::from(a.value == b.value),
                    }))
                }
                (Expr::Symbol(sym), [Expr::Literal(a), Expr::Literal(b)])
                    if sym.reference == "!=" =>
                ...
                (
                    ...
                    [
                        Expr::Meta(MetaExpr {
                            fragment: SExpr::List(list),
                            reference,
                        }),
                    ],
                ) if sym.reference == "cdr" => Ok(Expr::Meta(MetaExpr {
                    fragment: SExpr::List(list[1..].to_vec()),
                ...
                ) if sym.reference == "car" => Ok(Expr::Meta(MetaExpr {
                    fragment: list[0].clone(),
                    reference: reference.clone(),
                })),
                (
                    Expr::Symbol(sym),
                    [
                        Expr::Meta(MetaExpr { fragment, .. }),
                        Expr::Meta(MetaExpr {
                            fragment: SExpr::List(list),
                            ..
                        }),
                    ],
                ) if sym.reference == "cons" => {
                    let mut list = list.clone();
                    list.insert(0, fragment.clone());
                    Ok(Expr::Meta(MetaExpr {
                        fragment: SExpr::List(list),
                        reference: None,
                    }))
                }
                ...
                ) if sym.reference == "null?" => Ok(Expr::Literal(LiteralExpr {
                    value: u32::from(list.is_empty()),
                })),
                (
                    Expr::Symbol(sym),
                    [
                        Expr::Meta(MetaExpr {
                            fragment: SExpr::Char(a),
                            ..
                        }),
                        Expr::Meta(MetaExpr {
                            fragment: SExpr::Char(b),
                            ..
                        }),
                    ],
                ) if sym.reference == "char=" => Ok(Expr::Literal(LiteralExpr {
                    value: u32::from(a == b),
                })),
...
fn eval_continuation(
    continuation: &ContinuationExpr,
    continuation_id: u32,
    env: &HashMap<String, Expr>,
    global_env: &HashMap<String, Expr>,
    result: Result<Expr, JumpBuffer>,
) -> Result<Expr, JumpBuffer> {
    let continuation_id_expr = Expr::Literal(LiteralExpr {
    ...
    match result {
        Err(JumpBuffer {
            ...
            args,
            counter,
        }) if target_id == continuation_id && args.len() == continuation.parameters.len() => {
            ...
            let undefined_cont: Continuation = Rc::new(|_, _| {
                panic!("evaluation of the continuation expression should never return")
            });
            let result = eval(
...
pub fn eval(
    ...
    match expr {
        Expr::Symbol(sym) => cont(
            env.get(&sym.reference)
                .or_else(|| global_env.get(&sym.reference))
                .unwrap_or(expr),
            counter,
        ),
        Expr::If(ife) => eval(
            ...
            env,
            global_env,
            Rc::new(|condition, counter| match condition {
                Expr::Literal(value) if value.value != 0 => {
                    eval(&ife.consequent, counter, env, global_env, cont.clone())
                }
                Expr::Literal(_) => eval(&ife.alternate, counter, env, global_env, cont.clone()),
        ...
        Expr::With(with) => {
            ...
            match eval(
                ...
                Err(JumpBuffer {
                    ...
                    mut args,
                    counter,
                }) if target_id == continuation_id && args.len() == 1 => {
        ...
        Expr::Jump(jump) => eval_jump(jump, counter, env, global_env, &[]),
        Expr::Continuation(continuation) => {
            ...
            eval_continuation(
                ...
                env,
                global_env,
                cont(&continuation_id_expr, counter + 1),
            )
        }
        Expr::Invoke(invoke) => cont(
            &eval_invoke(invoke, counter, env, global_env, vec![])?,
            counter,
        ),
        ...
        Expr::Meta(meta) if meta.reference.is_some() => eval(
            expand(&mut expr.clone(), global_env),
            counter,
            env,
            global_env,
            cont,
...
pub fn expand<'a>(expr: &'a mut Expr, env: &HashMap<String, Expr>) -> &'a mut Expr {
    match expr {
        ...
        Expr::Meta(MetaExpr {
            ...
            let counter = 0;
            let reference = eval(reference, counter, &HashMap::new(), env, Rc::new(identity))
                .expect("jump must not escape macro expression");
            let arguments = vec![Expr::Meta(MetaExpr {
                reference: None,
                fragment: fragment.clone(),
            })];
            let expansion_expr = Expr::Invoke(InvokeExpr {
            ...
            .expect("jump must not escape macro");
